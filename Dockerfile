...
# The /roadmap page reads GITHUB_TOKEN at build time to fetch milestones
# + issues from the private mcp-rune/mcp-rune repo. Kamal forwards the
# secret via `builder.secrets` in config/deploy.yml; locally you can
# `docker build --secret id=GITHUB_TOKEN,src=<path>`. If the secret is
# absent, Astro falls back to the empty-state Roadmap and the rest of
# the site builds normally — that's why we tolerate a missing mount
# with `|| true`.
